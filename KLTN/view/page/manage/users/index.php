...
.role-manager {
    background: #e0f2fe; /* xanh dương nhạt */
    color: #1e3a8a;
    padding: 4px 8px;
    border-radius: 6px;
    font-weight: 600;
...
<div class="qlnl-header justify-content-end">
    ...
    $statusClass = $r['status'] == 1 
        ? 'status-working' 
        : 'status-left';

    $statusText = $r['status'] == 1 
        ? 'Đang làm việc' 
        : 'Nghỉ việc';
    ...
    <tr>
        ...
        <td>{$r['phone']}</td>
        <td><span class='role-badge $roleClass'>{$r['role_name']}</span></td>
        <td><span class='$statusClass'>$statusText</span></td>
        <td>{$r['warehouse_id']}</td>
        <td><a href='index.php?page=users//updateUsers&id={$r['user_id']}'><i class='fas fa-edit'></i></a></td>
    </tr>";
}
