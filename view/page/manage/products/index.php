...
<body>
	...
	<div class="container qlproduct">
		...
		if (is_array($products) && !empty($products)) {
			echo '<table><thead><tr>';
			echo '<th>Hình ảnh</th><th>Mã sản phẩm</th><th>Tên sản phẩm</th><th>Barcode</th><th>Loại</th><th>Nhà cung cấp</th><th>Kho</th><th>Tồn kho tối thiểu</th><th>Trạng thái</th><th>Thao tác</th>';
			echo '</tr></thead><tbody>';
			...
			foreach ($products as $product) {
				...
				echo "<tr>";
				echo "<td>{$imageTag}</td><td>{$id}</td><td>{$name}</td><td>{$barcodeImg}</td><td>{$type}</td><td>{$supplier}</td><td>{$warehouse}</td><td>{$min_stock}</td><td>" . ($status == 1 ? '<span class=\'status-active\'>Đang bán</span>' : '<span class=\'status-inactive\'>Ngừng bán</span>') . "</td><td>{$actionBtns}</td></tr>";
			}
			echo '</tbody></table>';
		} else {
