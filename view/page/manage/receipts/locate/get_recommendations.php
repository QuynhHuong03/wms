...
try {
    ...
    // ========================================
    $bins = [];
    $debugFilters = []; // Debug: track why bins are filtered out
    ...
    foreach ($zones as $zoneIndex => $zone) {
        ...
        foreach ($zone['racks'] as $rackIndex => $rack) {
            ...
            foreach ($rack['bins'] as $binIndex => $bin) {
                ...
                // Debug tracking
                $debugFilters[$binCode] = ['passed' => true, 'reason' => ''];
                ...
                // Reset capacity và product_id nếu bin trống
                if ($currentQty <= 0) {
                    $currentCapacityPercent = 0;
                    $binProductId = ''; // ✅ FIX: Reset product ID khi bin không còn hàng
                    $binProduct = [];   // ✅ FIX: Reset product data
                }
                ...
                if ($pSorted[0] > $bSorted[0] || $pSorted[1] > $bSorted[1] || $pSorted[2] > $bSorted[2]) {
                    $debugFilters[$binCode] = ['passed' => false, 'reason' => 'Physical fit - Product too large'];
                ...
                if ($binProductId) {
                    ...
                    // Chỉ áp dụng khi bin ĐÃ CÓ hàng
                    if (!$hasSameProduct && !$hasSameCategory) {
                        $debugFilters[$binCode] = [
                            'passed' => false, 
                            'reason' => 'Category mismatch - Bin has different category product',
                            'bin_product_id' => $binProductId,
                            'bin_category' => $binProductCategory ?: $binProductCategoryId,
                            'bin_quantity' => $currentQty,
                            'bin_capacity' => $currentCapacityPercent,
                            'current_product_id' => $productId,
                            'current_category' => $productCategory ?: $productCategoryId
                        ];
                        continue;
                    }
                }
                // Bin trống ($binProductId rỗng) -> chấp nhận cho tất cả sản phẩm
                
                // ========================================
                // FILTER 3: Capacity Check
                // ========================================
                if ($currentCapacityPercent >= 100) {
                    $debugFilters[$binCode] = ['passed' => false, 'reason' => 'Bin full - Capacity 100%'];
                    continue; // Bin full
                }
                ...
                if ($bestFit == 0) {
                    $debugFilters[$binCode] = ['passed' => false, 'reason' => 'Cannot fit - No valid orientation'];
                    continue; // Cannot fit
                }
                ...
                if ($itemsCanFit <= 0) {
                    $debugFilters[$binCode] = ['passed' => false, 'reason' => 'Items can fit = 0 (bestFit=' . $bestFit . ', currentQty=' . $currentQty . ', maxByVolume=' . $maxByVolume . ')'];
                    continue;
                }
    ...
    foreach ($bins as $bin) {
        ...
        // ⚠️ NOTE: Một bin có thể thuộc NHIỀU nhóm (không dùng elseif)
        ...
        // Category 2: Same Category (chỉ thêm nếu KHÔNG phải same product)
        if ($hasSameCategory && !$hasSameProduct) {
        ...
        // Bin có thể vừa là same_product/same_category VÀ optimal_fill
        if ($fillRateAfter >= 80 && $fillRateAfter <= 95) {
            $optimalFillBins[] = $bin;
        }
    }
    ...
    $response = [
        ...
        'debug_filters' => $debugFilters // Debug: show which bins were filtered and why
    ];
